...
async function runGodModeDemo() {
    ...
    console.log(`Initial Population: ${engine.storage.activeCount}`);

    // Disaster 1: Ice Age
    console.log("\n>>> Event: ICE AGE (Solar Constant = 0.1)");
    god.setSolarConstant(0.1);
    for (let i = 0; i < 100; i++) engine.update(dt);
    console.log(`Population after Ice Age: ${engine.storage.activeCount}`);
    ...
    // Count viruses (simplified for demo)
    console.log(`Current Population: ${engine.storage.activeCount}`);

    // Disaster 3: Extinction
    console.log("\n>>> Event: MASS EXTINCTION (90%)");
    god.triggerExtinction();
    console.log(`Final Population: ${engine.storage.activeCount}`);
